...
namespace glz
{
	...
	{
		class AI;
		typedef int RequestPriority;

		//priorities
		const static RequestPriority PriorityMax = 0;
		const static RequestPriority PriorityHigh = 64;
		const static RequestPriority PriorityNormal = 128;
		const static RequestPriority PriorityLow = 192;
		const static RequestPriority PriorityMin = 255;
		...
		*/
		...
		public:
			...
			*/
			 ...
			 */
			void seek(Vec2d pos, RequestPriority priority=PriorityNormal);

			/**
			* Creates a steering force that steers the agent directly away from the target position.
			*/
			void flee(Vec2d pos, RequestPriority priority = PriorityNormal);
			...
			*/
			void arrive(Vec2d pos, RequestPriority priority = PriorityNormal);

			/**
			* Seeks to a leading position to the target. If the agent is ahead of the target, then it
			* just seeks to the target.
			*/
			void pursuit(AI *target, RequestPriority priority = PriorityNormal);

			/**
			* Creates a steering force that makes the agent wander aimlessly.
			*/
			void wander(RequestPriority priority = PriorityNormal);
		};
	};
};
